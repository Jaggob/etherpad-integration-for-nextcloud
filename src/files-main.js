...
(function () {
	...
	const openCreatedPadInNativeViewer = async (navigation) => {
		if (hasNativeViewer() && navigation.path) {
			try {
				window.OCA.Viewer.open({ path: navigation.path })
				return
			} catch (e) {
				// Fall back to the route based opener below.
			}
		}
		await openPadInNativeViewer(navigation)
	}
	...
	const createInternalPublicPad = async () => {
		...
		try {
			...
			await openCreatedPadInNativeViewer({
				path: createdPath,
